const Room = {
  ...
  Room.setExits(Room.H2_1, Room.H1_1, Room.H2_2, Room.H3_1, Room.H6_2);
...
const findShortestPath = (start, end) => {
    ...
    while (queue.length > 0) {
        ...
        for (neighbour of currentRoom.getAllExits()) {
...
const reconstructPath = (path, start, end) => {
    const shortestPath = []
    let currentRoom = end
    while (currentRoom !== start) {
        shortestPath.push(currentRoom)
